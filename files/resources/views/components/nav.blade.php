...
<header id="header" class="sticky top-0 z-50 border-b border-line bg-canvas">
    <div class="flex h-16 items-stretch">
        ...
        <!-- Hovering the wordmark spins the spark and lifts the whole mark a touch; pressing squashes it down like a real button. -->
        <a href="/" aria-label="Homepage" class="group flex shrink-0 items-center border-r border-line px-5 text-ink sm:px-7">
            <span class="flex items-center gap-2.5 transition-transform duration-300 [transition-timing-function:var(--ease-spring)] group-hover:scale-[1.05] group-active:scale-[0.98]">
        ...
        <nav class="flex flex-1 items-center justify-end px-4 max-lg:hidden" aria-label="Main">
            <ul role="list" class="flex items-center gap-1 font-display text-[15px] font-medium">
                ...
                <li>
                    <!-- Hover rolls the label up and a twin rises from below; the current page carries a pink bar (see .nav-flip in site.css) -->
                    <a href="{{ $link->url }}" class="nav-flip-link relative block px-3.5 py-2">
                        <span class="nav-flip">
                            <span class="nav-flip-track">
                                <span>{{ $link->text }}</span>
